import Me from '../../assets/me.jpg';
import Tesla from '../../assets/Tesla_Clone.jpg';
import MGO from '../../assets/MGO_Final.png';


import { 
  createSlice,
  createAsyncThunk,
  createSelector,
  createEntityAdapter,
 } from '@reduxjs/toolkit'

const initialState = [
    {
        displayName: "Riley Fox",
        username: "rileyfox134",
        verified: true,
        title: "Developed this Twitter clone. This was my first Web Development project ever. I began learning how to use Javascript, React, HTML/CSS on this project. This project I tried incorporating Redux and React Router. Lastly, I wanted to redesign in into a resume format if I were to send a unique resume to Twitter. ",
        avatar: {Me},
        content: "data:image/jpeg;base64,", 
        comments: getRandomArbitrary(0,500),
        shares: getRandomArbitrary(0,500),
        likes: getRandomArbitrary(0,500),
        id: "1",          
      },
    {
        displayName: "Riley Fox",
        username: "rileyfox134",
        verified: true,
        title: "Recreated the Tesla Homepage. I created this project after my original Twitter clone to get more practice with Javasctipt, HTML, CSS. This project aided my ability to transfer a visual prebuilt design into code and helped me get more comfortable with React Functional Components.",
        avatar: {Me},
        content: Tesla, 
        comments: getRandomArbitrary(0,500),
        shares: getRandomArbitrary(0,500),
        likes: getRandomArbitrary(0,500),
        id: "2",          
    },
    {
        displayName: "Riley Fox",
        username: "rileyfox134",
        verified: true,
        title: "Made a fullstack C# website. This was my Capstone project for my Advanced Web-Development course in college. This taught and gave me exposure to web development using C#, Entity Framework, and using .Net overall. Gave me a different perspective as opposed to the other projects mentioned above.",
        avatar: {Me},
        content: MGO,
        comments: getRandomArbitrary(0,500),
        shares: getRandomArbitrary(0,500),
        likes: getRandomArbitrary(0,500),
        id: "3",       
      }
    
]

function getRandomArbitrary(min, max) {
  return Math.floor(Math.random() * (max - min) + min);
}

function uniqueTweetId() {
  return Math.floor(Math.random() * Date.now())
}

const postsSlice = createSlice({
    name: 'posts',
    initialState,
    reducers: {
      postAdded(state, action) {
        state.push(action.payload)
      }
    }
  })

  export const { postAdded } = postsSlice.actions

  export const selectAllPosts = state => state.posts

  export const selectPostById = (state, postId) =>
  state.posts.find(post => post.id === postId)

  
  export default postsSlice.reducer